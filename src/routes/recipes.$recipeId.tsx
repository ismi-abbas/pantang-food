  ...
  const favouriteRecipes = useMemo(
    () =>
      recipes
        .filter(
          (item: Recipe) =>
            favorites.includes(item.id) && item.id !== recipe.id,
        )
        .slice(0, 4),
    [favorites, recipe.id, recipes],
  )
  const shoppingList = useMemo(
    () => buildShoppingList(recipes, [recipe.id, ...favorites]),
    [favorites, recipe.id, recipes],
  )
    ...
    <div className="bg-[var(--pantang-bg)] text-[var(--pantang-ink)]">
      ...
      <main className="mx-auto max-w-[1040px] px-4 pb-16 pt-6 sm:px-6 lg:px-8">
        <Button
          asChild
          variant="ghost"
          className="mb-4 h-8 rounded-full px-2 text-[11px] uppercase tracking-[0.16em] text-[var(--pantang-muted)] hover:bg-[var(--pantang-warm)] hover:text-[var(--pantang-ink)]"
        >
          <Link to="/">
        ...
        <section className="mt-6 grid gap-4 lg:grid-cols-[0.95fr_1.05fr] lg:items-start">
          <div className="border-b border-[var(--pantang-line)] pb-6">
            <div
              className={`overflow-hidden rounded-[1.4rem] border border-[var(--pantang-line)] bg-[var(--pantang-deep)]`}
            >
              <div className="flex items-center justify-between gap-4 p-4 pb-0">
                <Badge
                  variant="outline"
                  className="rounded-full border-[var(--pantang-line)] bg-[var(--pantang-cream)] px-3 py-2 text-[10px] uppercase tracking-[0.18em] text-[var(--pantang-muted)]"
                >
                ...
                <Button
                  ...
                  className={`rounded-2xl border ${isFavourite ? 'border-[var(--pantang-terra)] bg-[color:oklch(0.565_0.14_43_/_10%)] text-[var(--pantang-terra-deep)]' : 'border-[var(--pantang-line)] bg-[var(--pantang-cream)] text-[var(--pantang-muted)]'}`}
                  ...
                  {recipe.imageUrl ? (
                    ...
                    <div className="flex h-full items-center justify-center p-6 text-center">
                      <div>
                        ...
                        <p className="mt-3 text-lg leading-8 text-[var(--pantang-soft)]">
                          {recipe.title} served warm for a gentle recovery meal.
                        </p>
                      </div>
                    </div>
                  )}
          ...
          <div className="space-y-6">
            ...
            <p className="mt-5 max-w-[58ch] text-[1.05rem] leading-8 text-[var(--pantang-soft)]">
              {recipe.summary}
            </p>
            <div className="grid gap-3 border-t border-[var(--pantang-line)] pt-6 sm:grid-cols-2 xl:grid-cols-4">
              ...
              <MetaStat
                label="Saved with"
                value={`${favorites.length + 1} recipes`}
          ...
          <aside className="space-y-6 lg:sticky lg:top-24">
            <div className="border-t border-[var(--pantang-line)] pt-8">
              ...
              <ul className="mt-4 space-y-3">
                {recipe.ingredients.map((item) => (
                  <li
                    key={item}
                    className="flex gap-3 border-b border-[var(--pantang-line)] py-3 text-sm leading-7 text-[var(--pantang-ink)] last:border-b-0"
                  >
                    <span
                      className="mt-1 h-2 w-2 rounded-full bg-[var(--pantang-terra)]"
                      aria-hidden="true"
                    />
                    <span>{item}</span>
                  </li>
            ...
            <div className="border-t border-[var(--pantang-line)] pt-8">
              ...
              <p className="mt-3 text-sm leading-7 text-[var(--pantang-muted)]">
                This combines the current recipe with your saved favourites.
              </p>
          ...
          <div className="space-y-6">
            <div className="border-t border-[var(--pantang-line)] pt-8">
              ...
              <div className="mt-4 space-y-1">
                {recipe.steps.map((item, index) => (
                  <div
                    key={item}
                    className="grid gap-4 border-b border-[var(--pantang-line)] py-5 sm:grid-cols-[56px_1fr] last:border-b-0"
                  >
                    <div className="text-4xl leading-none text-[var(--pantang-terra-deep)]">
                      {String(index + 1).padStart(2, '0')}
                    </div>
                    <div>
                      <p className="text-[1rem] leading-8 text-[var(--pantang-ink)]">
                        {item}
                      </p>
            ...
            <div className="grid gap-6 xl:grid-cols-2 border-t border-[var(--pantang-line)] pt-8">
              <section className="border-t border-[var(--pantang-line)] pt-8">
                <h2 className="text-2xl font-semibold tracking-[-0.03em] text-[var(--pantang-ink)]">
                  Kitchen notes
                </h2>
                <div className="mt-4 space-y-2">
                  {recipe.pantryTips.map((tip: string) => (
                    <div
                      key={tip}
                      className="border-t border-[var(--pantang-line)] pt-3 text-sm leading-7 text-[var(--pantang-soft)]"
                    >
                      {tip}
                    </div>
                  ))}
              ...
              <section className="border-t border-[var(--pantang-line)] pt-8">
                <h3 className="text-2xl font-semibold tracking-[-0.03em] text-[var(--pantang-ink)]">
                  {wisdom.title}
                </h3>
                <p className="mt-3 text-sm leading-7 text-[var(--pantang-soft)]">
                  {wisdom.body}
                </p>
              </section>
            </div>
              ...
              <section className="border-t border-[var(--pantang-line)] pt-8">
                ...
                <div className="mt-4 space-y-3">
                  {relatedRecipes.map((item: Recipe) => (
                    ...
                    >
                      <p className="font-medium text-[var(--pantang-ink)]">
                        {item.title}
                      </p>
              ...
              <section className="border-t border-[var(--pantang-line)] pt-8">
                ...
                <div className="mt-4 space-y-3">
                  {favouriteRecipes.length > 0 ? (
                    favouriteRecipes.map((item: Recipe) => (
                      <Link
                        key={item.id}
                        to="/recipes/$recipeId"
                        params={{ recipeId: item.id }}
                    ...
                    <p className="text-sm leading-7 text-[var(--pantang-muted)]">
                      Save a few recipes and your favourites will appear here.
                    </p>
                  )}
                </div>
              </section>
